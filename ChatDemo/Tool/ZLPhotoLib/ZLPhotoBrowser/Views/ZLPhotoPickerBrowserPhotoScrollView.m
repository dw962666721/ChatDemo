...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 0){
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"保存成功啦" delegate:self cancelButtonTitle:nil otherButtonTitles:nil];
        [alertView show];
        NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:1.5f target:self selector:@selector(performDismiss:) userInfo:alertView repeats:NO];
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            [_photoImageView sd_setImageWithURL:photo.photoURL placeholderImage:thumbImage options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                ...
                if (image == nil)
                {
                    [_photoImageView sd_setImageWithURL:photo.photoURL placeholderImage:thumbImage];
                }
                else
                {
...
- (void)imageView:(UIImageView *)imageView doubleTapDetected:(UITouch *)touch {
    ...
    // 加载失败啦png图片不添加双击手势。
    if (image.size.height==72&&image.size.width==73.5)
    {
        return;
    }
