...
//初始化函数
-(void) loadKey:(NSString *)app_id mch_id:(NSString *)mch_id;
{
    //初始构造函数
...
//设置商户密钥
-(void) setKey:(NSString *)WXAppKey mch_id:(NSString *)mchid key:(NSString *)key notifyURL:(NSString *)notifyURL orderno:(NSString *)orderno productDescription:(NSString *)productDescription amount:(NSString *)amount
{
    wxAppid   = [NSString stringWithString:WXAppKey];
    MCH_ID   = [NSString stringWithString:mchid];
    payUrl     = @"https://api.mch.weixin.qq.com/pay/unifiedorder";
    spkey  = [NSString stringWithString:key];
    NotifyURL = [NSString stringWithString:notifyURL];
    Orderno = [NSString stringWithString:orderno];
    ProductDescription = [NSString stringWithString:productDescription];
    Amount = [NSString stringWithString:amount];
}
//获取debug信息
...
//    NSString *orderno   = [NSString stringWithFormat:@"%ld",time(0)];
    ...
    [packageParams setObject: ProductDescription        forKey:@"body"];        //订单描述，展示给用户
...
//    [packageParams setObject: @"196.168.1.1"    forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: [[NSString alloc] initWithFormat:@"%d",int(Amount.doubleValue*100)] forKey:@"total_fee"];       //订单金额，单位为分
